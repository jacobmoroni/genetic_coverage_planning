...
class PathMaker(object):
    ...
    def smartly_place_dots(self):
        ...
        displacements = np.array([obstacles[None, 0] - XY[:,0, None], obstacles[None, 1] - XY[:,1, None]])
        ...
        print(closest.shape)
        idx = distances.T[closest]<self._safety_buffer
        print(idx.shape)
        XY[idx,0] + (self._safety_buffer-distances[idx])*np.cos(angles[idx])
        XY[idx,1] - (self._safety_buffer-distances[idx])*np.sin(angles[idx])
