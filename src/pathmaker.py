...
class PathMaker(object):
    ...
    def smartlyPlaceDots(self):
        ...
        XY_scale[idx, 0] = XY_scale[idx, 0] - \
            (self._safety_buffer-min_distances[idx])*np.cos(min_angles[idx])
        XY_scale[idx, 1] = XY_scale[idx, 1] - \
            (self._safety_buffer-min_distances[idx])*np.sin(min_angles[idx])
        ...
        idx_bool = waypoint_distances < \
            self._hall_width*self._waypoint_dist_factor
        ...
        for wp in pruning_idx:
            ...
            XY_scale[wp[0], 1] = XY_scale[wp[0], 1] - \
                (XY_scale[wp[0], 1] - XY_scale[wp[1], 1])/2
            XY_scale[wp[1]] = -1
    ...
    def computeTraversableGraph(self):
        ...
        for ii, distance_row in enumerate(distances):
            ...
            for sector in range(len(sectors)):
                candidates = np.array(np.where(sectored_angles[ii] ==
                                               sector)).flatten()
                if sector == 4:
                    candidates = candidates[candidates != ii]
                if candidates.size > 0:
                    traversable_nodes.append(
                        candidates[np.argmin(distance_row[candidates])])
        ...
        for ii, edge in enumerate(tqdm(in_range_idx.T, desc="Prune Collisions")):
            if self._mappy.lineCollisionCheck(self._XY[edge[0]],
                                              self._XY[edge[1]],
    ...
    def makeMeAPath(self, path_len, start_idx, prev_path=None):
        ...
        for _ in range(path_len):
            ...
            if len(choices_comb) > 0:
                w = self.assignWeights(current_heading,
                                       current_idx,
                                       choices_comb)
